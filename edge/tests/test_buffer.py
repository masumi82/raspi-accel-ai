...
def test_add_enforces_max_entries_drops_oldest(tmp_path):
    buf = OfflineBuffer(str(tmp_path / "b.jsonl"), max_entries=2)
    buf.add({"a": 1})
    buf.add({"b": 2})
    buf.add({"c": 3})
    assert buf.pending() == [{"b": 2}, {"c": 3}]
...
def test_pending_skips_corrupt_line(tmp_path):
    path = str(tmp_path / "b.jsonl")
